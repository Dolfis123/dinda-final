...
const PegawaiWithoutShift: React.FC = () => {
  ...
  const fetchPegawai = async () => {
    try {
      const response = await axios.get(
        'http://localhost:5000/api/pegawai/without-shift',
      );
  ...
  return (
    ...
    >
      <h1>Pegawai / Karywan Tanpa Shift</h1>
      {isLoading ? (
        <p>Loading...</p>
      ) : (
          ...
          <div className="bg-white p-6 rounded shadow-lg w-1/2">
            ...
            <form onSubmit={handleSave}>
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  <label className="block text-gray-700">Tipe Karyawan</label>
                  <select
                    className="border rounded w-full py-2 px-3"
                    name="tipe_karyawan"
                    value={currentPegawai?.tipe_karyawan || ''}
                    onChange={handleInputChange}
                    disabled={modalAction === 'view'}
                    required
                  >
                    <option value="">Pilih Tipe Karyawan</option>
                    <option value="Dengan Shift">Dengan Shift</option>
...
export default PegawaiWithoutShift;
